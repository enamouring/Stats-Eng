...
plt.hist(numbers,100); #plotting the amount of numbers in a histogram ie. 1 has 1 occurrence 1-[1], 2-[2],...
...
plt.hist(X,100); #plotting data from previous line
...
for i in range(1,P.shape[0]):
    ...
    for j in range(1,P.shape[1]-1): #nested loop

        P[i,j] = P[i-1,j-1]+P[i-1,j] #algorithm for Pascal's triangle
...
def var(x): #takes the mean of the previous result
...
plt.plot(y) #normal curve

num_bins = 50
n,bins,patches = plt.hist(x, num_bins, density=1)
y = ((1 / (np.sqrt(2 * np.pi) * sigma)) * np.exp(-0.5 * (1 / sigma * (bins - mu))**2))
plt.plot(bins,y) #plot together
...
X = np.random.random((1000000,10)) #explain this for hw
